...
#define LOA__IMPLS(name, ns, table_t, key_t, val_t, hashfn, cmpfn, alloc, dealloc) \
    ...
    int ns ## resize_fast(table_t* t, uint32_t newsize) {                          \
        ...
        loa_key_t(name) *keys, *okeys = t->keys;                                   \
        loa_val_t(name) *vals, *ovals = t->vals;                                   \
        ...
        return 0;                                                                  \
                                                                                   ...
                                                                                   \
    loa_result_t ns ## put(table_t* t, loa_key_t(name) key) {                      \
        loa_result_t rv;                                                           \
        if (t->size >= t->cutoff) {                                                \
        ...
        __builtin_unreachable();                                                   \
                                                                                   ...
                                                                                   \
    loa_iter_t ns ## get(const table_t* t, key_t key) {                            \
        const loa_key_t(name)* keys = t->keys;                                     \
...
#define LOA_TABLE_INIT(name, key, val, hash, compfn, alloc, dealloc) \
    LOA__TYPES(name, loa_##name##_table, key, val)                   \
    LOA__IMPLS(name, loa_##name##_, loa_##name##_table, key, val, hash, compfn, alloc, dealloc)
