...
#define loa_key_t(name)         loa_ ## name ## _key_t
#define loa_val_t(name)         loa_ ## name ## _val_t
#define loa_table_t(name)       loa_ ## name ## _table
...
#define loa_next(name, t, k)    loa_ ## name ## _next(t, k)
#define loa_begin(name, t)      loa_next(name, t, 0)
#define loa_exists(t, k)        loa__live(t->msks, k)
...
#define LOA__TYPES(name, table_t, key_t, val_t) \
    typedef key_t loa_key_t(name);              \
    typedef val_t loa_val_t(name);              \
...
#define LOA__PROTOS(ns, table_t, key_t, val_t)                    \
    ...
    int          ns ## initialize();                              \
    void         ns ## finalize();                                \
    loa_iter_t   ns ## next(table_t* t, loa_iter_t k);            \
...
#define LOA__IMPLS_EX(ns, table_t, key_t, val_t, hashfn, cmpfn, alloc, dealloc) \
    ...
    void ns ## destroy(table_t* t) {                                            \
        ...
        free(t);                                                                \
                                                                                ...
                                                                                \
    loa_iter_t ns ## next(table_t* t, loa_iter_t k) {                           \
        do { ++k; } while (k != loa_end(t) && !loa_exists(t, k));               \
        return k;                                                               \
    }                                                                           \
                                                                                \
    int ns ## resize_fast(table_t* t, uint32_t newsize) {                       \
...
#define LOA__IMPLS(name, hash, compfn, alloc, dealloc) \
    LOA__IMPLS_EX(loa_##name##_, loa_table_t(name), loa_key_t(name), loa_val_t(name), hash, compfn, alloc, dealloc)
